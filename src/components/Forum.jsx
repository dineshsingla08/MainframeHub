...
export const Forum = ({ user, token, triggerAuthModal }) => {
  ...
  const fetchTopics = async () => {
    setLoading(true);
    try {
      const res = await fetch((window.API_BASE || '') + '/api/forum/topics');
  ...
  const handleSelectTopic = async (topic) => {
    setLoading(true);
    try {
      const res = await fetch((window.API_BASE || '') + `/api/forum/topics/${topic.id}`);
  ...
  const handleAddReply = async (e) => {
    ...
    try {
      const res = await fetch((window.API_BASE || '') + `/api/forum/topics/${selectedTopic.id}/replies`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const handleLike = async (topicId, e) => {
    e.stopPropagation();
    try {
      const res = await fetch((window.API_BASE || '') + `/api/forum/topics/${topicId}/like`, {
        method: 'POST'
      });
      if (res.ok) {
